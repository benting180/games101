...
{   
    ...
    bool con1 = v01.cross(v0p).z() > 0;
    bool con2 = v12.cross(v1p).z() > 0;
    bool con3 = v20.cross(v2p).z() > 0;
        ...
        for (int y = ymin; y <= ymax; ++y)
        {
            ...
            for (int i=0; i<=1; ++i)
            {
                for (int j=0; j<=1; ++j)
                {
                    ...
                    float x_super = x + 0.25 + 0.5 * i;
                    float y_super = y + 0.25 + 0.5 * j;
                    if ( insideTriangle(x_super, y_super, t.v) ) {
                        numWithin += 1;
                        color += t.getColor() * 0.25;
                    
                        auto[alpha, beta, gamma] = computeBarycentric2D(x_super, y_super, t.v);
                        float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                        float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                        z_interpolated *= w_reciprocal; 
                        if ( z_interpolated < z_small )
                        {
                            z_small = z_interpolated;
                        }
                    }
                }
            }
            if ( (numWithin > 0) && ( z_small < depth_buf[get_index(x, y)] ) )
            {
                depth_buf[get_index(x, y)] = z_small;
